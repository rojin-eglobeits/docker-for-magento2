...
default_log="${project_dir}/log/debug.log"
log_file_path="${project_dir}/scripts/.current_log_path"
nesting_level_file="${project_dir}/scripts/.current_nesting_level"
...
function log() {
    ...
    if [[ -n "${input}" ]]; then
        if [[ -f "${log_file_path}" ]]; then
            log_file="${project_dir}/$(cat "${log_file_path}")"
        else
...
function initLogFile()
{
    ...
    rm -f "${project_dir}/log/${log_file}.log"
}
